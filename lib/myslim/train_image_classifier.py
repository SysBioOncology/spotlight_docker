...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(deploy_config.inputs_device()):
            ...
            labels = tf.Print(
                labels, [labels], summarize=FLAGS.batch_size, message="labels"
            )
            # t_ps  Tensor("batch:2", shape=(64,), dtype=int64, device=/device:CPU:0)
            t_ps = tf.abs(
                tf.divide(tf.cast(t_ps, tf.float32), tf.cast(100, tf.float32))
            )

            neg_ls = tf.divide(
                tf.subtract(tf.cast(1, tf.float32), t_ps),
                tf.cast(dataset.num_classes - FLAGS.labels_offset - 1, tf.float32),
            )
            neg_M = tf.multiply(
                tf.expand_dims(neg_ls, 1),
                tf.ones(
                    [FLAGS.batch_size, dataset.num_classes - FLAGS.labels_offset],
                    dtype=tf.float32,
                ),
            )

            onehot_tmp = tf.multiply(
                tf.expand_dims(tf.subtract(t_ps, neg_ls), 1),
