...
def compute_node_degree_with_es(tile_quantification_path,
                                cell_types=None,
                                graphs_path=None,
                                shapiro_alpha=0.05,
    ...
    ###############################################
    nd_results = Parallel(n_jobs=n_cores)(delayed(features.node_degree_wrapper)(
    ...
    # Testing normality
    shapiro_tests = Parallel(n_jobs=n_cores)(delayed(utils.test_normality)(
        sims_nd=all_sims_nd,
        slide_submitter_id=id,
        alpha=shapiro_alpha,
        cell_types=cell_types) for id in all_sims_nd.slide_submitter_id.unique())
    all_shapiro_tests = pd.concat(shapiro_tests, axis=0)

    # Computing Cohen's d effect size and perform t-test
    effect_sizes = Parallel(n_jobs=n_cores)(delayed(features.compute_effect_size)(
    ...
    return all_effect_sizes, all_sims_nd, all_mean_nd_df, example_simulations, all_shapiro_tests, all_graphs


def main(args):
    all_effect_sizes, all_sims_nd, all_mean_nd_df, example_simulations, all_shapiro_tests, all_graphs = compute_node_degree_with_es(
    ...
    all_effect_sizes.to_csv(
        Path(args.output_dir, f"{args.prefix}_features_ND_ES.csv"), sep="\t", index=False)
    all_sims_nd.to_csv(
        Path(args.output_dir, f"{args.prefix}_features_ND_sims.csv"), sep="\t", index=False)
