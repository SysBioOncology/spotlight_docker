...
def compute_colocalization(tile_quantification_path,
                           cell_types=None,
                           graphs_path=None,
                           abundance_threshold=0.5, n_cores=multiprocessing.cpu_count()):
    """
...
def main(args):
    ...
    all_dual_nodes_frac.to_csv(
        Path(args.output_dir,
             f"{args.prefix}_features_coloc_fraction.csv"), sep="\t", index=False)

    colocalization_wide.to_csv(
        Path(args.output_dir,
             f"{args.prefix}_features_coloc_fraction_wide.csv"), sep="\t", index=False)
