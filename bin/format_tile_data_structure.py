...
def assess_tcga_slide_quality(slide_name, slides_folder):
    ...
    img = openslide.OpenSlide(
        "{}/{}".format(slides_folder, slide_name))
...
def format_tile_data_structure(slides_folder, tiles_folder, output_dir, clinical_file_path, is_tcga=True):
    """
    ...
    if is_tcga:
        for slide_name in jpg_tiles_df.image_file_name.unique():
            slide_quality.append(assess_tcga_slide_quality(slide_name = slide_name, slides_folder = slides_folder))
    else:
        jpeg_quality = 100 # assuming no loss
        slide_quality = [[slide_name, f"RGB{jpeg_quality}"] for slide_name in jpg_tiles_df.image_file_name.unique()]
...
def main(args):
    ...
    output.to_csv(Path(args.output_dir, "file_info_train.txt"),
                  index=False, sep="\t")
