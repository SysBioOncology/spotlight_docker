...
CMD ["bash", "-c", "/run_scripts/1_extract_histopatho_features.sh /data_example/images /data/checkpoint/Retrained_Inception_v4/model.ckpt-100000 ${WSI_TYPE} /output_example"]
...
CMD ["bash", "-c", "/run_scripts/2_tile_level_cell_type_quantification.sh /output_example/1_histopathological_features ${WSI_TYPE} /output_example"]

# 3. Compute spatial graph-based interpretable features (arguments: celltype_tile_quantifications, slide_type, output folder)

CMD ["bash", "-c", "run_scripts/3_compute_spatial_features.sh output_example/2_tile_level_quantification/test_tile_predictions_proba.csv ${WSI_TYPE} /output_example"]
