...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device(deploy_config.inputs_device()):
            ...
            t_ps = tf.abs(tf.divide(tf.cast(t_ps, tf.float32),
                          tf.cast(100, tf.float32)))

            neg_ls = tf.divide(tf.subtract(tf.cast(1, tf.float32), t_ps), tf.cast(
                dataset.num_classes - FLAGS.labels_offset - 1, tf.float32))
