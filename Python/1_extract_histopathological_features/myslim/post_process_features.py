...
def post_process_features(output_dir, slide_type, data_source="TCGA"):
    """
    ...
    if slide_type == "FF":
        features_raw = pd.read_csv(
            output_dir + "/bot_train.txt", sep="\t", header=None)
        ...
        features["Coord_Y"] = [i[-1]
                               for i in features["tile_ID"].str.split("_")]
        # FIX add sample_submitter_id and slide_submitter_id depending on data_source
        if (data_source == "TCGA"):
        ...
        # TODO TESTING move function to utils and convert to def instead of lambda
        ...
        features.to_parquet(path=OUTPUT_PATH, compression='gzip',
...
if __name__ == "__main__":
    ...
    post_process_features(output_dir=args.output_dir,
                          slide_type=args.slide_type, data_source=args.data_source)
