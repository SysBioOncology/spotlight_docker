...
def tile_level_quantification(models_dir, output_dir, var_names_path, histopatho_features_dir, prediction_mode="all", n_outerfolds=5, cell_types_path="", slide_type="FF"):
    """
    ...
    if slide_type == "FF":
        FEATURES_PATH = f"{histopatho_features_dir}/features.txt"
        histopatho_features = pd.read_csv(FEATURES_PATH, sep="\t", index_col=0)
    elif slide_type == "FFPE":
        FEATURES_PATH = f"{histopatho_features_dir}/features_format_parquet"
    ...
    tile_predictions.to_csv(
        f"{full_output_dir}/{prediction_mode}_tile_predictions_zscores.csv", sep="\t", index=False)
    pred_proba.to_csv(
        f"{full_output_dir}/{prediction_mode}_tile_predictions_proba.csv", sep="\t", index=False)


if __name__ == "__main__":

    parser = argparse.ArgumentParser(
        description="Predict cell type abundances for the tiles")
    ...
    parser.add_argument("--histopatho_features_dir", type=str,
                        help="Path to histopathological features file", required=True)
