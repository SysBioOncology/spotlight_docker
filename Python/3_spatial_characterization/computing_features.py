...
def compute_network_features(tile_quantification_path, output_dir, slide_type=DEFAULT_SLIDE_TYPE, cell_types=None, graphs_path=None,
    ...
    colocalization_wide = all_dual_nodes_frac.pivot(
        index=["slide_submitter_id"], columns="pair")["frac"]
    new_cols = [
        f'Co-loc {col.replace("_", " ")} clusters' for col in colocalization_wide.columns]
    ...
    all_largest_cc_sizes_wide.to_csv(
        f"{output_dir}/{slide_type}_features_lcc_fraction.csv", sep="\t", index=False)
    proximity_graphs.to_csv(
        f"{output_dir}/{slide_type}_features_shortest_paths_thresholded.csv", sep="\t", index=False)
    all_dual_nodes_frac.to_csv(
        f"{output_dir}/{slide_type}_features_coloc_fraction.csv", sep="\t", index=False)
    all_features.to_csv(
        f"{output_dir}/{slide_type}_all_graph_features.csv", sep="\t", index=False)
    ...
    nd_results = Parallel(n_jobs=NUM_CORES)(delayed(features.node_degree_wrapper)(
        all_graphs[id], predictions, id) for id in slide_submitter_ids)
    ...
    # Testing normality
    shapiro_tests = Parallel(n_jobs=NUM_CORES)(delayed(utils.test_normality)(sims_nd=all_sims_nd, slide_submitter_id=id,
                                                                             alpha=shapiro_alpha, cell_types=cell_types) for id in all_sims_nd.slide_submitter_id.unique())
    all_shapiro_tests = pd.concat(shapiro_tests, axis=0)

    # Computing Cohen's d effect size and perform t-test
    effect_sizes = Parallel(n_jobs=NUM_CORES)(delayed(features.compute_effect_size)(
        all_mean_nd_df, all_sims_nd, slide_submitter_id) for slide_submitter_id in all_sims_nd.slide_submitter_id.unique())
    ...
    all_shapiro_tests.to_csv(
        f"{output_dir}/{slide_type}_shapiro_tests.csv", index=False, sep="\t")
...
def compute_clustering_features(
    ...
    slide_clusters = Parallel(n_jobs=NUM_CORES)(delayed(clustering.schc_all)(
    ...
    slide_indiv_clusters = Parallel(n_jobs=NUM_CORES)(delayed(clustering.schc_individual)(
        predictions, all_graphs[id], id) for id in slide_submitter_ids)
    ...
    clusters_all_schc_long = all_slide_clusters_characterized.melt(
        id_vars=["slide_submitter_id", "cluster_label"], value_name="is_assigned", var_name="cell_type")
    ...
    prox_all_schc = pd.merge(prox_all_schc, clusters_all_schc_long, left_on=[
                             "slide_submitter_id", "cluster1"], right_on=["slide_submitter_id", "cluster_label"])
    prox_all_schc = prox_all_schc.rename(
        columns={"cell_type": "cluster1_label"})
    prox_all_schc = prox_all_schc.drop(columns=["cluster_label"])

    prox_all_schc = pd.merge(prox_all_schc, clusters_all_schc_long, left_on=[
                             "slide_submitter_id", "cluster2"], right_on=["slide_submitter_id", "cluster_label"])
    prox_all_schc = prox_all_schc.rename(
        columns={"cell_type": "cluster2_label"})

    # Order doesn't matter: x <->
    prox_all_schc["pair"] = [f"{sorted([i, j])[0]}-{sorted([i, j])[1]}" for i,
                             j in prox_all_schc[["cluster1_label", "cluster2_label"]].to_numpy()]
    prox_all_schc = prox_all_schc[((prox_all_schc.cluster1 == prox_all_schc.cluster2) & (
    ...
    # Post Processing
    results_schc_all = Parallel(n_jobs=NUM_CORES)(delayed(features.post_processing_proximity)(
        prox_df=prox_all_schc, slide_submitter_id=id, method="all") for id in slide_submitter_ids)
    ...
    # Between clusters
    slide_submitter_ids = list(set(predictions.slide_submitter_id))
    results_schc_indiv = Parallel(n_jobs=NUM_CORES)(delayed(features.compute_proximity_clusters_pairs)(all_slide_indiv_clusters, slide_submitter_id=id, method="individual_between",
                                                                                                       n_clusters=n_clusters, cell_types=cell_types, max_dist=max_dist, max_n_tiles_threshold=max_n_tiles_threshold, tile_size=tile_size, overlap=overlap) for id in slide_submitter_ids)
    ...
    # Post-processing
    slide_submitter_ids = list(set(predictions.slide_submitter_id))
    results_schc_indiv = pd.concat(Parallel(n_jobs=NUM_CORES)(delayed(features.post_processing_proximity)(
        prox_df=prox_indiv_schc, slide_submitter_id=id, method="individual_between") for id in slide_submitter_ids))

    # Within clusters
    slide_submitter_ids = list(set(predictions.slide_submitter_id))
    results_schc_indiv_within = Parallel(n_jobs=NUM_CORES)(delayed(features.compute_proximity_clusters_pairs)(all_slide_indiv_clusters, slide_submitter_id=id, method="individual_within",
                                                                                                              n_clusters=n_clusters, cell_types=cell_types, max_dist=max_dist, max_n_tiles_threshold=max_n_tiles_threshold, tile_size=tile_size, overlap=overlap,) for id in slide_submitter_ids)
    ...
    results_schc_indiv_within = pd.concat(Parallel(n_jobs=NUM_CORES)(delayed(features.post_processing_proximity)(
    ...
    prox_indiv_schc_combined = pd.concat(
        [results_schc_indiv_within, results_schc_indiv])
...
# slide_submitter_ids))
    ...
    all_prox_df_wide = all_prox_df.pivot(
        index=["slide_submitter_id"], columns=["pair"])["proximity"]
    new_cols = [
    ...
    tiles_all_schc = tiles_all_schc.drop(axis=1, columns=cell_types)
    all_slide_indiv_clusters = all_slide_indiv_clusters.drop(
        axis=1, columns=cell_types)  # drop the predicted probabilities
    ...
    tiles_all_schc.to_csv(
        f"{output_dir}/{slide_type}_all_schc_tiles.csv", sep="\t", index=False)
    all_slide_clusters_characterized.to_csv(
        f"{output_dir}/{slide_type}_all_schc_clusters_labeled.csv", sep="\t", index=False)
    all_slide_indiv_clusters.to_csv(
        f"{output_dir}/{slide_type}_indiv_schc_tiles.csv", sep="\t", index=False)
    slide_indiv_clusters_labeled.to_csv(
        f"{output_dir}/{slide_type}_indiv_schc_clusters_labeled.csv", sep="\t", index=False)
    all_prox_df.to_csv(
        f"{output_dir}/{slide_type}_features_clust_all_schc_prox.csv", sep="\t", index=False)
    prox_indiv_schc_combined.to_csv(
        f"{output_dir}/{slide_type}_features_clust_indiv_schc_prox.csv", sep="\t", index=False)
    # shape_feature_means.to_csv(f"{output_dir}/{slide_type}_features_clust_shapes.csv", sep="\t", index=False)
    all_features.to_csv(
        f"{output_dir}/{slide_type}_clustering_features.csv", sep="\t", index=False)
...
def post_processing(output_dir, slide_type="FF", metadata_path="", is_TCGA=False, merge_var="slide_submitter_id", sheet_name=None):
    ...
    """
    all_features_graph = pd.read_csv(
        f"{output_dir}/{slide_type}_all_graph_features.csv", sep="\t")
    all_features_clustering = pd.read_csv(
        f"{output_dir}/{slide_type}_clustering_features.csv", sep="\t")
    ...
    all_features_combined.to_csv(
        f"{output_dir}/all_features_combined.csv", sep="\t", index=False)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Derive spatial features")
    parser.add_argument("--workflow_mode", type=int,
                        help="Choose which steps to execute: all = 1, graph-based = 2, clustering-based = 3, combining features = 4 (default: 1)", default=1)
    ...
    parser.add_argument("--output_dir", type=str,
                        help="Path to output folder to store generated files", required=True)
    ...
    parser.add_argument("--cell_types_path", type=str,
                        help="Path to file with list of cell types (default: CAFs, endothelial_cells, T_cells, tumor_purity)", default="")
    ...
    parser.add_argument("--max_dist", type=int,
                        help="Maximum distance between clusters", required=False, default=None)
    parser.add_argument("--max_n_tiles_threshold", type=int,
                        help="Number of tiles for computing max. distance between two points in two different clusters", default=2, required=False)
    parser.add_argument("--tile_size", type=int,
                        help="Size of tile (default: 512)", default=512, required=False)
    parser.add_argument("--overlap", type=int,
                        help="Overlap of tiles (default: 50)", default=50, required=False)

    parser.add_argument("--metadata_path", type=str,
                        help="Path to tab-separated file with metadata", default="")
    parser.add_argument("--is_TCGA", type=bool,
                        help="dataset is from TCGA (default: True)", default=True, required=False)
    parser.add_argument("--merge_var", type=str,
                        help="Variable to merge metadata and computed features on", default=None)
    ...
    # FIX create output directory for saving
    ...
    if workflow_mode == 1:
        ...
        post_processing(output_dir=output_dir, slide_type=slide_type, metadata_path=metadata_path,
                        is_TCGA=False, merge_var=merge_var, sheet_name=sheet_name)
        ...
        raise Exception(
            "Invalid workflow mode, please choose one of the following (int): all = 1, graph-based = 2, clustering-based = 3, combining features = 4 (default: 1)")
